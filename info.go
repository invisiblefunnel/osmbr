...
func decodeDenseInfo(data []byte, info *DenseInfoBuf) error {
	...
	for msg.Next() {
		...
		case 2: // timestamp (packed int64, delta)
			info.Timestamps, err = msg.RepeatedInt64(info.Timestamps)
		case 3: // changeset (packed int64, delta)
			info.Changesets, err = msg.RepeatedInt64(info.Changesets)
		case 4: // uid (packed int32, delta)
			info.UIDs, err = msg.RepeatedInt32(info.UIDs)
